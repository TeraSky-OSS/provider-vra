...
// BlockDeviceSnapshot is the Schema for the BlockDeviceSnapshots API. Creates a VMware Aria Automation block device snapshot resource.
...
type BlockDeviceSnapshot struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.blockDeviceId) || (has(self.initProvider) && has(self.initProvider.blockDeviceId))",message="spec.forProvider.blockDeviceId is a required parameter"
